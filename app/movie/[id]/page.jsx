...
async function getMovie(params){
    const res = await fetch(`https://api.themoviedb.org/3/movie/${params}?api_key=b9e4f85c3b6987a76b17bb7579bc1ac8`)
    const data = await res.json()
    return data
}

async function getMovieCredits(params){
    const res = await fetch(`https://api.themoviedb.org/3/movie/${params}/credits?api_key=b9e4f85c3b6987a76b17bb7579bc1ac8`)
    const data = await res.json()
    return data
}
        ...
        <div className='relative max-w-[1440px] mx-auto p-4'>
            <div className='flex md:flex-row flex-col gap-4'>
                ...
                <div className='flex flex-col gap-2'>
                    <h2 className='text-2xl font-bold'>{movie.title} <span>{movie.release_date.slice(0,4)}</span></h2>
                    <p>{movie.overview}</p>
